// -----------------------------------------------------------------------------
// Passive ability script: PassiveAbility_150015Log
// Namespace/file: ruina-roguelike-reborn-main\abcdcode_LOGLIKE_MOD\PassiveAbility_150015Log.cs
...
/// <summary>Passive ability: PassiveAbility_150015Log</summary>

public class PassiveAbility_150015Log : PassiveAbilityBase
{
  public int _patternCount;
  public List<LorId> _usedCardType = new List<LorId>();
  public int _onlyCardCooltime;

  public override void OnWaveStart() => this._usedCardType = new List<LorId>();
  ...
  public void SetCards()
  {
    ...
    if (TheIndexCardUtilLog.IsActivatedRelease(this.owner))
    {
      ...
      {
        this.AddNewCard(new LorId(LogLikeMod.ModId, 605002));
    ...
    if (speedDiceList.Count >= 4)
      num = speedDiceList.Count - 3;
    if (num <= 0)
      return;
  ...
  public bool CheckOtherNamed()
  {
    ...
    foreach (BattleUnitModel battleUnitModel in aliveList)
    {
      ...
      if (battleUnitModel.passiveDetail.HasPassive<PassiveAbility_150017Log>())
      {
        flag = true;
        break;
      }
