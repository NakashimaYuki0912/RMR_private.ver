...
AssertContains 'font rendering check guards TMP replacement' $logLike 'CanTmpFontRenderText'
...
AssertContains 'RMR TMP font sanitizer rewrites existing UI text' $patches 'string sanitizedText = RewardingModel.SanitizeDisplayText(text.text)'
AssertContains 'RMR TMP font sanitizer writes sanitized text back' $patches 'text.text = sanitizedText'
AssertContains 'RMR TMP font replacement is guarded by glyph support' $patches 'ShouldUseRmrTmpFont'
AssertContains 'RMR TMP font replacement preserves renderable vanilla fonts' $patches '!LogLikeMod.CanTmpFontRenderText(currentFont, text)'
