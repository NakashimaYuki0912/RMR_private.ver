...
- **普通异想体书页也写入永久图鉴**（随游玩获得/选中记录），供解放战过程中的异想体书页选择；图鉴解锁不等于新开路线自动塞入路线持有。
- 解放战**首次**胜利后的专属异想体书页和 E.G.O. 书页写入永久图鉴/永久奖励池；已通关再战不重复发奖、不重复写入。
- 解放战临时编队与选页池使用永久图鉴已解锁的四类内容（核心页、战斗书页、异想体、E.G.O.）；战后必须恢复当前路线配置。
- 图鉴 UI 中异想体与 E.G.O. **不做**都市章节分段，扁平展示；角色/战斗书页可保留章节分段。

### 3.3 解放战入口与战斗配置

- 解放战唯一入口是**开局主菜单**（`RMRStartHubPanel`）的「挑战解放战」；初始遗物事件与编队界面不得再提供解放入口。
- 玩家选择「正常游玩」后本局关闭解放入口；放弃路线进度并重新开局后可再次从主菜单挑战。主菜单阶段可按楼层多次进入解放战（已通关楼层可再战练手）。
- 其他事件、章节、按钮或直接方法调用均不得绕过门控（`CanEnterRealizationFromHub`）。
...
- 解放战**首次**胜利记录对应楼层完成状态，将该层专属异想体与 E.G.O. 写入永久图鉴，并永久开放该层专属奖励池。
- 奖励不是未通关前直接出现，也不是把整层全部普通异想体页在首胜时整包塞进路线持有。
- 完成后，专属奖励才可进入后续商店或战斗结算的随机候选池；仍需去重、未拥有和升级版过滤。
- 已通关楼层可再挑战；再胜不入队奖励 UI、不重复写图鉴。
- 已知 E.G.O. 奖励 ID 段：
